...
import { FileText, Search, Bot, Send, BarChart, AlertTriangle, HelpCircle, RefreshCw, Download } from "lucide-react";
...
const mockReportData: Report = {
  ...
  type: "rfp-analysis",
  summary: "This RFP for Enterprise Resource Planning System is generally well-structured but has several compliance gaps that should be addressed before finalization. The document meets 92% of standard compliance requirements, with notable issues in data security specifications, accessibility standards, environmental compliance, vendor certification requirements, and SLA definitions.",
  score: 92,
  findings: [
    {
  ...
  chatHistory: [
    {
      id: "1",
      role: "system",
      content: "I've analyzed the Enterprise Resource Planning System RFP document. What would you like to know about it?",
    ...
    },
    {
      id: "2",
      role: "user",
      content: "What are the main compliance issues you found?",
      timestamp: "2023-07-20T10:31:00Z"
    },
    {
      id: "3",
      role: "system",
      content: "The main compliance issues I found are:\n\n1. Data Security: NIST 800-53 is mentioned but the specific revision is not specified, which could lead to ambiguity.\n\n2. Accessibility: While ADA compliance is mentioned, there's no explicit reference to Section 508 requirements, which are essential for public sector procurement.\n\n3. Environmental Compliance: The RFP lacks specific references to e-waste regulations for hardware disposal.\n\n4. Vendor Certification: ISO 27001 certification is required, but the timing requirements are unclear (whether vendors need to be certified at bid submission or can obtain certification later).\n\n5. SLA Terms: The service level agreement terms lack specific response and resolution time metrics categorized by severity levels.",
      agent: "Compliance AI Agent",
      model: "GPT-4",
      provider: "openai",
      timestamp: "2023-07-20T10:31:30Z"
    }
  ]
};
  ...
  const sendMessage = () => {
    ...
    setTimeout(() => {
      ...
      const aiResponse: ChatMessage = {
        ...
        role: "system",
        content: "Based on my analysis, I can provide more specific details about the document. The compliance score is 92%, which is good but there are several areas that need improvement. Would you like me to elaborate on any specific finding or recommendation?",
        agent: selectedAgent.name,
        model: selectedModel?.name || "GPT-4",
        provider: selectedModel?.provider || "openai",
        timestamp: new Date().toISOString()
        ...
        <TabsContent value="document-selection" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Choose the document you want to analyze with AI
              </CardDescription>
            </CardHeader>
              ...
              <div>
                ...
                {selectedDocuments.length > 0 ? (
                  <div className="p-3 border rounded-md bg-blue-50">
                    ...
                    <p className="text-sm text-muted-foreground">{selectedDocuments[0].referenceNumber}</p>
                  </div>
                ) : (
            ...
            <Card className="lg:col-span-2">
              ...
              <CardContent className="p-0">
                ...
                <div className="p-4 flex space-x-2">
                  ...
                  <Button
                    onClick={sendMessage}
                    disabled={!newMessage.trim() || chatMessages.length === 0}
                  >
                            ...
                            <div key={index} className="border rounded-md p-2">
                              <div className="flex justify-between items-center mb-1">
                                ...
                                <div className="flex gap-1">
                                  <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${complianceColors[finding.compliance]}`}>
                                    {finding.compliance}
                                  </span>
                                  <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${severityColors[finding.severity]}`}>
                                    {finding.severity}
                                  </span>
                                </div>
                              </div>
